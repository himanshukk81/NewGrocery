  ...
  showToastMessage(messageText)
  {
   ...
   createUser(userInfo): Observable<Users[]> {
		 let data=this.convertToParams(userInfo)
				...
				return this.httpClient.post(this.API_URL+"register",data,httpOptions).map((response: Response)=>{
					return response;
				})
		...
		getOperatingLocations():Observable<Users[]>{
			return this.httpClient.get(this.API_URL+"getOperatingLocations").map((response: Response)=>{
				return response;
			})
			.catch(this.handleError);	
		}
		...
		getCommunities():Observable<Users[]>{
			return this.httpClient.get(this.API_URL+"getCommunities").map((response: Response)=>{
				return response;
			})
			.catch(this.handleError);	
		}
